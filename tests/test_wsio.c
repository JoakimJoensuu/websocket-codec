...
static wsio_result pump_out(wsio *from, wsio *to)
{
    size_t nbytes;
    ...
    r.n = 0;
    if (nbytes) {
        r = wsio_feed(to, p, nbytes);
        EXPECT(r.err == WSIO_OK || wsio_error(to) != WSIO_OK);
...
static void test_64k_binary(void)
{
    ...
    r = wsio_feed(srv, frame, flen);
    EXPECT(r.err == WSIO_OK);
    EXPECT(r.n == 1);
    EXPECT(r.evs[0].kind == WSIO_EV_BIN);
...
static void test_incomplete_utf8_at_fin(void)
{
    ...
    EXPECT(wsio_feed(srv, frame, n).err == WSIO_ERR_UTF8);
...
static void test_roundtrip_text(void)
{
    ...
    r = pump_out(cli, srv);
    EXPECT(r.n == 1);
    EXPECT(r.evs[0].kind == WSIO_EV_TEXT);
    EXPECT(r.evs[0].len == 5);
    EXPECT(memcmp(r.evs[0].data, msg, 5) == 0);

    EXPECT(wsio_send_text(srv, r.evs[0].data, r.evs[0].len) == WSIO_OK);
    r = pump_out(srv, cli);
    EXPECT(r.n == 1);
    EXPECT(r.evs[0].kind == WSIO_EV_TEXT);
    EXPECT(r.evs[0].len == 5);
...
static void test_fragmentation_and_ping(void)
{
    ...
    EXPECT(r.err == WSIO_OK);
    EXPECT(r.n == 1);
    EXPECT(r.evs[0].kind == WSIO_EV_PING);
...
static void test_utf8_split_and_invalid(void)
{
    ...
    EXPECT(wsio_feed(srv, frame, n).err == WSIO_OK);
...
static void test_protocol_errors(void)
{
    ...
    EXPECT(wsio_feed(srv, frame, n).err == WSIO_ERR_PROTOCOL);
    ...
    /* masked to client */
    {
        wsio *cli = wsio_create(WSIO_ROLE_CLIENT);
        n = build_frame(frame, 1, WSIO_OP_TEXT, 1, hi, 1, KEY);
        EXPECT(wsio_feed(cli, frame, n).err == WSIO_ERR_PROTOCOL);
        wsio_destroy(cli);
    }
}
...
static void test_close(void)
{
    ...
    EXPECT(r.err == WSIO_OK);
    EXPECT(r.n == 1);
    EXPECT(r.evs[0].kind == WSIO_EV_CLOSE);
    ...
    EXPECT(r.n == 1);
    EXPECT(r.evs[0].kind == WSIO_EV_CLOSE);
    EXPECT(r.evs[0].close_code == WSIO_CLOSE_NO_STATUS);
    wsio_destroy(srv);
...
static void test_too_big(void)
{
    ...
    EXPECT(wsio_feed(srv, frame, n).err == WSIO_ERR_TOO_BIG);
...
static void test_non_minimal_length(void)
{
    ...
    EXPECT(wsio_feed(srv, frame, 9).err == WSIO_ERR_PROTOCOL);
    wsio_destroy(srv);
}
...
static void test_two_messages_one_feed(void)
{
    ...
    r = wsio_feed(srv, both, n1 + n2);
    EXPECT(r.err == WSIO_OK);
    EXPECT(r.n == 2);
    EXPECT(r.evs[0].kind == WSIO_EV_TEXT);
    EXPECT(r.evs[0].len == 3);
    EXPECT(memcmp(r.evs[0].data, "one", 3) == 0);
    EXPECT(r.evs[1].kind == WSIO_EV_TEXT);
